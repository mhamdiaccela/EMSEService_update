...
function builRecordlist(arr) {
	...
	htmlOutput += '<tr><th></th><th>Contract Number</th><th>Commericial Name </th><th>Plan Number</th>';
	htmlOutput += '<th>Block Number</th> <th>Plot ID </th>  <th>Governorate </th> <th>Kuwait Area</th> <th>Case File Barcode</th></tr>';
...
function selectPLCA() {
	...
	try {
		if (globalVars.selectedConsignee.length == 0) {
			alert("Please select a PLCA");
			return;
		}
		if (globalVars.recordType == "CHPO") {

			winParent.document.getElementById("app_spec_info_CURRENTCONTRACTDETAILS_currentContractLicenseNumber").value = globalVars.selectedConsignee;
			...
			winParent.document.getElementById("app_spec_info_INSPECTIONDETAILS_plotID").value = selectedRow.plotID;
			winParent.focus();
			window.close();
			// 
			...
			winParent.document.getElementById("app_spec_info_REQUESTDETAILS_currentContractNumber").value = globalVars.selectedConsignee;
			winParent.document.getElementById("app_spec_info_REQUESTDETAILS_allocationEntity").value = selectedRow.allocationEntity;
			winParent.document.getElementById("app_spec_info_REQUESTDETAILS_plotID").value = selectedRow.plotID;
			winParent.document.getElementById("app_spec_info_REQUESTDETAILS_surfaceArea").value = selectedRow.surfaceArea;
